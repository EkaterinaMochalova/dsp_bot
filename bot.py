...
# ВАЖНО: не используем Command() тут.
# Игнорируем любые сообщения, начинающиеся с "/" (это команды),
# берём только обычный текст не от ботов.
nlu_router.message.filter(F.text, ~F.text.regexp(r"^/"), ~F.via_bot)

# --- utils: парсинг и извлечение подсказок ---
